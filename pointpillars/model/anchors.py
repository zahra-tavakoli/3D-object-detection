...
def anchor_target(batched_anchors, batched_gt_bboxes, batched_gt_labels, assigners, nclasses):
    ...
    for i in range(batch_size):
        ...
        for class_id in range(n_assigners): # one anchor group per class
            assigner = assigners[class_id]
            pos_iou_thr, neg_iou_thr, min_iou_thr = \
            assigner['pos_iou_thr'], assigner['neg_iou_thr'], assigner['min_iou_thr']
            cur_anchors = anchors[:, :, class_id, :, :].reshape(-1, 7)
            assigned_gt_inds = -torch.ones_like(cur_anchors[:, 0], dtype=torch.long)
            class_mask = gt_labels == class_id
            class_gt_bboxes = gt_bboxes[class_mask]
            class_gt_labels = gt_labels[class_mask]

            if class_gt_bboxes.numel() == 0:
                assigned_gt_inds[:] = 0
            else:
                overlaps = iou2d_nearest(class_gt_bboxes, cur_anchors)
                ...
                for gt_idx in range(len(class_gt_bboxes)):
                    if gt_max_overlaps[gt_idx] >= min_iou_thr:
                        best_anchor_mask = overlaps[gt_idx] == gt_max_overlaps[gt_idx]
                        assigned_gt_inds[best_anchor_mask] = gt_idx + 1
            ...
            assigned_gt_labels[pos_flag] = class_gt_labels[
            ...
            corr_gt_bboxes = class_gt_bboxes[assigned_gt_inds[pos_flag] - 1]
            if positive_anchors.numel() > 0:
